...
const trendTrackerService = {
     ...
     */
    async fetchTimeSeries(searchTerm, timeWindows, githubToken, metric = 'repositories') {
        if (!githubToken) {
        ...
        try {
            ...
            return this.processTimeSeriesData(result.data, timeWindows, metric);
        } catch (error) {
     ...
     */
    processTimeSeriesData(data, timeWindows, metric) {
        const timeSeriesData = [];
        
        timeWindows.forEach((window, index) => {
            const periodAlias = `period${index}`;
            const timePoint = {
                period: this.formatPeriodLabel(window),
 ...
 generateTimeWindows(startYear, endYear, granularity = 'year') {
     ...
     */
    async calculateWindowedHIndex(searchTerm, startDate, endDate, property, githubToken) {
        if (!githubToken) {
        ...
        // Calculate H-Index using the existing utility function
     ...
     */
    async compareSearchTerms(searchTerms, timeWindows, metric, githubToken) {
        const results = {};
        
        // Fetch data for each search term
        for (const term of searchTerms) {
            try {
                const termData = await this.fetchTimeSeries(term, timeWindows, githubToken, metric);
